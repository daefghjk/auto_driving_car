...
#define MOTION_DRIVER_TARGET_MSP430		//定义驱动部分,采用MSP430的驱�??(移�?�到STM32F4)
...
//陀螺仪方向设置
static signed char gyro_orientation[9] = { 1, 0, 1,
                                           0, 1, 0,
                                           0, 0, 1};
//MPU6050�??测试
//返回�??:0,正常
...
//    printf("res = %d\r\n",res);
	if(res==0)	//初�?�化MPU6050
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置所需要的传感�??
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//设置FIFO
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//设置采样�??
		if(res)return 3; 
...
//得到dmp处理后的数据(注意,�??函数需要比较�?�堆�??,局部变量有点�??)
//pitch:�??仰�?? 精度:0.1°   范围:-90.0° <---> +90.0°
//roll:�??滚�??  精度:0.1°   范围:-180.0°<---> +180.0°
//yaw:�??向�??   精度:0.1°   范围:-180.0°<---> +180.0°
//返回�??:0,正常
//    其他,失败
